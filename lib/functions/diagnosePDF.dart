...
class diagnosePDF extends StatefulWidget {

  final String pdf;
  const diagnosePDF({Key key, this.pdf,this.result,this.testName}) : super(key: key);
  final String result,testName;
  @override
  _PDFViewerState createState() => _PDFViewerState();
}
String pdf;
class _PDFViewerState extends State<diagnosePDF> {
  ...
  Future<String> loadPDFurl() async{
    try{
      print(widget.result);
      pdf = await firebaseStorage.ref().child('Diagnosis').child(widget.result+'.pdf').getDownloadURL();
      return pdf;
    }
    catch(e){
      pdf = null;
